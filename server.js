...
app.put("/clients/:id", async (req, res) => {
  ...
  try {
    ...
    if (password && password.trim() !== "") {
      await tx.query("UPDATE clients SET login = $1, name = $2, password = $3 WHERE id = $4", [login.toUpperCase(), name, password, clientId]);
    } else {
      await tx.query("UPDATE clients SET login = $1, name = $2 WHERE id = $3", [login.toUpperCase(), name, clientId]);
    }
...
app.post("/products", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT p.id, p.name, p.category, p.group_id, pg.name AS group_name
       FROM products p
